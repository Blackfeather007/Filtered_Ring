...
variable {R : Type u} [CommRing R] {A : Type w} [Ring A] [Algebra R A]

variable (F : ι → Submodule R A) [FilteredAlgebra F]

def Filtration.LTSubmodule (i : ι) : Submodule R (F i) := (⨆ k < i, F k).comap (F i).subtype

instance Filtration.LTSetoid (i : ι) : Setoid (F i) :=
  Submodule.quotientRel (Filtration.LTSubmodule F i)
...
@[simp]
lemma Filtration.hMul_coe {i j : ι} (x : F i) (y : F j) : ((x * y : F (i + j)) : A) = x * y := rfl
...
lemma Filtration.lt_mul_mem {i j : ι} {x y} (hx : x ∈ ⨆ k < i, F k) (hy : y ∈ F j) :
  ...
  | add x y hx hy hxy hyy =>
    rw [add_mul]
    have := Set.add_mem_add hxy hyy
    simp only [coe_add_coe, SetLike.mem_coe] at this
    exact this
  | mem s y h =>
    rename_i y'
    have := @Submodule.mem_iSup_of_mem R A _ _ _ ι (y * y') F (i + j)
    have := FilteredAlgebra.mul_mem
    exact Submodule.mem_iSup_of_mem ⟨i + j, add_lt_add_right s.2 _⟩ (FilteredModule.mul_mem h hy)
    sorry
